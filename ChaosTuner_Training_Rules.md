...
5. **NETWORK APIS**: Native functions `http_get(url)`, `ws_connect`, etc. are available. Any usage of these MUST include instructions to the user to update host config with `[plugins.<plugin>]` `granted_capabilities` (e.g. `net_http`) and `[plugins.<plugin>.permissions]` `http` / `net_allowlist` before running the plugin.
6. **DATABASE APIS**: Functions like `db_connect` and `redis_connect` MUST instruct the user to grant `db_external` and set `[plugins.<plugin>.permissions]` `sql` / `sql_urls` (never bare `*` unless opt-in).
