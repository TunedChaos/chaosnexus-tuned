...
Optimized for IBM Granite 4.1-8B LoRA fine-tuning using Unsloth on AMD ROCm (RX 7900 XTX 24GB VRAM).
Base model aligned with Crucible default Tuned GGUF (Granite 4.1-8B), not Qwen.
"""
...
def main():
    ...
    model_name = "unsloth/granite-4.1-8b"  # matches Crucible/Models.md / train_peft_rocm
    ...
    def format_prompts(examples):
        # Granite chat template via tokenizer (not Qwen <|im_start|> markers).
        texts = []
        role_map = {
            "system": "system",
            "human": "user",
        ...
        for conversations in examples["conversations"]:
            ...
            for message in conversations:
                role = role_map.get(message.get("from", "user"), "user")
    ...
    save_path = os.path.join(os.path.dirname(__file__), "models", "chaosnexus-tuned-granite-4.1-8b-lora")
